...
def find_num(idx):                                   # 2진수로 구성된 암호를 불러와서 그 행에 배수를 계산해 올바른 형식의 암호는 result 리스트에 넣는다. 
    sentence = bi_arr[idx]                           # 이 sentence에는 몇개의 암호가 들어있는지 알수 없다. -> 남은 문장들은 다시 확인해봐야 할것이다.
    ...
    while True:                                      # 배수가 결정날때까지 계속 확인
        ...
        if ck and compare in pattern_lst:            # 똑같은 pattern이 나올때 까지 배수를 늘린다. key_error 방지
            ...
            if rest:                                 # rstrip하고 남은것이 빈문자열이 아니면
                bi_arr.append(rest)                  # bi_arr에 추가해서 계속 더할수 있게 만든다.
            return
        else:
            cnt += 1                                 # 배수를 추가한다.(1~35)
    ...
    for i in range(len(arr)):
        if arr[i]:
            ...
            bi_arr.append(bi_data.rstrip('0'))      # 2진수 안에 있는 오른쪽 0을 모두 제거하기        # 000000000000000000000000000000000001110110110001011101101100010110001000110100100110111011(rstrip)
                                                    ...
                                                    # 없으면 1자리씩 띄워서 뽑아왔을 때 이것이 pattern_lst에 있으면 배수는 2
    for i in range(len(bi_arr)):
        find_num(i)
    value = 0
    result = list(set(result))
    for j in range(len(result)):                    # 마지막으로 검증코드를 확인해야한다.
        value += safe(result[j])
    print(f'#{t} {value}')